...
<body style="width:100%;height:14.5%;margin:0px 0px 0px 0px">
	<form name="editForm" action="EditUser" method="get">	
	<div class="content-area-right"><h2 class="entry-title"><span>EDIT USER </span>/ ADD USER FORM</h2>
	<div class="site-content edit-add-user">
	<div class="edit-left-input">
	<input type="hidden" name="userId" value="${requestScope.copyUser ? requestScope.userId : requestScope.user.userId}"></input>
	<input type="hidden" name="saveUser" value="false"></input>
	...
	<p class="Email"><label>Email</label><input type="text" name="email" value="${requestScope.copyUser ? requestScope.mail : requestScope.user.email}"></p>
	<p class="Mobile"><label>Mobile</label><input type="text" name="mobile" value="${requestScope.user.mobile}"></p>
	<p class="Collage"><label>Collage</label><input type="text" name="college" value="${requestScope.user.college}"></p>
	...
	<div class="submit-upld"><button type="submit" onclick="javascript:doSave()">Save</button><button>Cancel</button><button>Reset</button></div>
	</div><!---- .site-content ---->
